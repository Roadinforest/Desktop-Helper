...
/*void SaveRemind(Noderm *Rnew)
{
    ...
    //数据按照添加先后，所以为“存储”+“新增”
    //1.获取存储文件中的数据。
    //2.接收的Rnew是一个新结点，接在已有之后
    //3.重新写入文件
    //-----------------------读取文件-----------------------
    Noderm* Head = NULL;//用于接收FileRM
    Noderm* Current = NULL;//辅助结点
    Noderm* Each = new Noderm;
    int i = 0;
    //创建对象
    QFile file(Filenamerm);
    QDataStream in(&file);
    if(!file.open(QIODevice::ReadOnly))//打开同时判断
    {
        qDebug()<<"文件打开失败";
    }
    while(!in.atEnd())
    ...
    file.close();
    //----------------------接收新数据-----------------------
    if(Rnew)//虽然我的代码已经保证了Rnew不为空，但是老子稳健/赞
    {
        qDebug()<<"you";
        Rnew->next = Head;//虽然上面说的是已有+新建，但是初次使用的时候已有为空，一玩空指针就会爆炸
    }
    else
    {
        qDebug()<<"wu";
        return ;//如果啥也没，那就直接不用保存了，毁灭吧
    }
    qDebug()<<"好了，走到这里就代表两段数据被连起来了";
...
Noderm* RemindFetch()
{
    ...
    if (in.atEnd())//防止用户初次使用时文件为空导致返回空指针
    {
