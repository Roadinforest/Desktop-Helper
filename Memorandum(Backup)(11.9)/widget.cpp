    ...
    , ui(new Ui::Widget)
{
    ...
    //----------------------------课程部分初始化------------------------------
    ui->Xedit->setValidator(new QRegularExpressionValidator(QRegularExpression("[0-9]{1}")));
    ...
    ui->CourseTable->setRowCount(CourseNums);//宏常量便于设置更改
...
void Widget::ShowCourse()
{
    ...
    while(Current)
    {
        ...
        int x = Current->val.GetX();
        qDebug()<<"周"<<x;
        int y1 = Current->val.GetY1();
        qDebug()<<"第"<<y1<<"节课开始";
        int y2 = Current->val.GetY2();
        qDebug()<<"第"<<y2<<"节课开始";
        ...
        for(int i = y1; i <= y2; i++)
        {
            ...
            //qDebug()<<"error2";
            //ui->CourseTable->item(i,x)->setFont(QFont("华文行楷",12));
            //qDebug()<<"error3";
...
void Widget::ShowHomework()
{
    ...
    while(Current)
    {
        ...
        for(int i = 0; i <= 5; i++)
        {
            if(i==3)
            {
                ui->HomeworkTable_1->item(row, i)->setFont(QFont("华文行楷",12));
                continue;
            }
            ui->HomeworkTable_1->item(row, i)->setFont(QFont("华文行楷",14));
        }
        Current = Current->next;
    }
    ui->HomeworkTable_1->update();
...
void Widget::ShowRemind()
{
    ...
    while(Current)
    {
        ...
        for(int i = 0; i <= 2; i++)
        {
            ui->RemindTable->item(row, i)->setFont(QFont("华文行楷",14));
        }
        //qDebug()<<"d";
        //qDebug()<<i<<":"<<Current;
        Current = Current->next;
